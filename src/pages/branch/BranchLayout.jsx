...
import { LayoutDashboard, Users, ShoppingBag, RotateCcw, X, LogOut, Menu, Bell, ArrowLeftRight, Package, UserCircle, BarChart2, Settings } from "lucide-react";
...
const navItems = [
  ...
  { path: "/branch/reports",   label: "Analytics",  icon: BarChart2 },
    ...
    return (
      <Link
        ...
        style={{
          ...
          fontSize: 13,
          background: active ? "linear-gradient(135deg,#059669,#0d9488)" : "transparent",
...
export default function BranchLayout() {
  ...
  const initials = userProfile
    ...
    : "BM";
  const displayName = userProfile ? `${userProfile.firstName} ${userProfile.lastName}` : "Branch Manager";
  ...
  const SidebarInner = ({ showClose }) => (
    <>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #d1fae5", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        {showClose && (
          <button onClick={() => setSidebarOpen(false)} style={{ border: "none", background: "none", cursor: "pointer", color: "#8a909c" }}>
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button className="lg-sidebar" onClick={() => setSidebarOpen(true)} style={{ border: "none", background: "none", cursor: "pointer", display: "none" }}>
              <Menu size={20} />
            </button>
            <div>
              <p style={{ fontWeight: 700, fontSize: 15, margin: 0 }}>{branch?.name ?? "Branch Dashboard"}</p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#059669,#0d9488)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 11, fontWeight: 700 }}>
              {initials}
            </div>
            <div>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#1a1d23" }}>{displayName}</p>
              <p style={{ margin: 0, fontSize: 11, color: "#6b7280" }}>{displayEmail}</p>
            </div>
          </div>
        </header>
