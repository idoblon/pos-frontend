...
export default function OperationsCenter() {
  ...
  const data = useMemo(() => ({
    branches: collection(branches), employees: collection(employees), orders: collection(storeOrders),
    refunds: collection(refundsByStore), shifts: collection(shiftsByStore),
  ...
  const health = useMemo(() => data.branches.map((branch) => {
    ...
    const orders = data.orders.filter((order) => branchId(order) === currentBranchId);
    const refunds = data.refunds.filter((refund) => branchId(refund) === currentBranchId);
    const shifts = data.shifts.filter((shift) => branchId(shift) === currentBranchId);
    ...
    let score = 100;
    const reasons = [];
    if (!orders.length) { score -= 20; reasons.push("No recorded sales"); }
    ...
    return { branch, score: Math.max(score, 0), sales, orders: orders.length, refunded, reasons };
  }), [data]);
  ...
  return (
    <div className="space-y-5 p-5">
      <div><h1 className="text-xl font-bold text-gray-950">Operations Center</h1><p className="mt-1 text-sm text-gray-500">Branch health and cashier exceptions, calculated from current POS activity.</p></div>
      {loading ? <p className="text-sm text-gray-500">Loading operations data…</p> : <>
        <section className="rounded-xl border bg-white">
          <div className="flex items-center gap-2 border-b p-4"><HeartPulse size={18} /><div><h2 className="font-semibold">Branch health</h2><p className="text-xs text-gray-500">A 100-point score based on recorded sales, refund rate, and open overtime shifts.</p></div></div>
