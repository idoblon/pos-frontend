...
function StoreSubscriptionCard({ store, onViewDetails, onManageSubscription }) {
  ...
  const plan = SUBSCRIPTION_PLANS[store.subscriptionPlan] || SUBSCRIPTION_PLANS.BASIC;
  const statusStyle = SUBSCRIPTION_STATUS[store.subscriptionStatus] || SUBSCRIPTION_STATUS.ACTIVE;
  
  const daysUntilExpiry = store.subscriptionExpiry 
    ? Math.ceil((new Date(store.subscriptionExpiry) - new Date()) / (1000 * 60 * 60 * 24))
    : 0;
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
          <span style={{ fontSize: "12px", fontWeight: "600", color: plan.color }}>
            {plan.name} Plan
          </span>
        ...
        {store.subscriptionExpiry && (
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <span style={{ fontSize: "11px", color: "#6b7280" }}>
              Expires: {new Date(store.subscriptionExpiry).toLocaleDateString()}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ margin: 0, fontSize: "10px", color: "#6b7280" }}>Branches</p>
        </div>
        <div style={{ textAlign: "center" }}>
          <p style={{ margin: 0, fontSize: "16px", fontWeight: "700", color: "#1a1d23" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <p style={{ margin: 0, fontSize: "16px", fontWeight: "700", color: plan.color }}>
            रु {(store.monthlyRevenue || 0).toLocaleString("en-IN")}
          </p>
          <p style={{ margin: 0, fontSize: "10px", color: "#6b7280" }}>Revenue</p>
        </div>
      </div>
...
export default function StoreSubscriptionOverview() {
  ...
  const stats = {
    ...
    activeSubscriptions: storesWithSubscriptions.filter(s => s.subscriptionStatus === "ACTIVE").length,
    expiringSoon: storesWithSubscriptions.filter(s => s.subscriptionStatus === "EXPIRING_SOON").length,
    totalRevenue: storesWithSubscriptions.reduce((sum, store) => sum + (store.monthlyRevenue || 0), 0)
  };
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px" }}>
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              ...
              <span style={{ fontSize: "12px", fontWeight: "600", color: "#6b7280" }}>Total Stores</span>
            </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              ...
              <span style={{ fontSize: "12px", fontWeight: "600", color: "#6b7280" }}>Active Subscriptions</span>
            </div>
            <p style={{ margin: 0, fontSize: "18px", fontWeight: "700", color: "#1a1d23" }}>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", fontSize: "12px" }}>
              ...
              <div><strong>Store Name:</strong> {selectedStore.name}</div>
              <div><strong>Address:</strong> {selectedStore.address}</div>
              <div><strong>Phone:</strong> {selectedStore.storePhone}</div>
              <div><strong>Email:</strong> {selectedStore.storeEmail}</div>
              <div><strong>Subscription Plan:</strong> {SUBSCRIPTION_PLANS[selectedStore.subscriptionPlan].name}</div>
              <div><strong>Status:</strong> {SUBSCRIPTION_STATUS[selectedStore.subscriptionStatus].text}</div>
              <div><strong>Expiry:</strong> {selectedStore.subscriptionExpiry ? new Date(selectedStore.subscriptionExpiry).toLocaleDateString() : "N/A"}</div>
              <div><strong>Last Payment:</strong> {selectedStore.lastPayment ? new Date(selectedStore.lastPayment).toLocaleDateString() : "N/A"}</div>
              <div><strong>Branches:</strong> {selectedStore.branchCount}</div>
              <div><strong>Employees:</strong> {selectedStore.employeeCount}</div>
              <div><strong>Monthly Revenue:</strong> रु {(selectedStore.monthlyRevenue || 0).toLocaleString("en-IN")}</div>
            </div>
